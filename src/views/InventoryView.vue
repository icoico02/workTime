    ...
    <main class="inventory-content">
      ...
      <template v-else-if="currentSection === 'documents'">
        ...
        <div class="business-flow-list">
          <article v-for="item in filteredLogs.filter((row) => row.business_no)" :key="item.id" :class="`document-row-type-${item.operation_type}`" @click="openFlow(item)">
