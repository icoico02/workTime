...
const sections = [['dashboard', '总览'], ['checkout', '开单'], ['orders', '订单'], ['products', '商品'], ['inbound', '入库'], ['outbound', '出库'], ['returns', '退货'], ['stock', '库存'], ['logs', '流水'], ['documents', '单据']]
const currentSection = computed(() => route.params.section || 'dashboard')
const sectionTitle = computed(() => ({ dashboard: '进销存总览', checkout: '快速开单', orders: '销售订单', products: '商品管理', inbound: '入库登记', outbound: '出库登记', returns: '销售退货', stock: '库存状态', logs: '经营流水', documents: '业务单据' }[currentSection.value] || '进销存总览'))
...
async function loadData() {
  ...
  const [productRes, movementRes, orderRes, returnRes, roleRes] = await Promise.all([
    ...
    supabase.from('inventory_orders').select('*, inventory_order_items(id,product_name,quantity,unit_price,source_supplier_id,actual_cost,line_total,line_profit,inventory_suppliers(name))').order('created_at', { ascending: false }).limit(500),
...
async function completeSale() {
  ...
  try {
    ...
    if (selectedSources.length) {
      const { data: orderItems, error: itemsError } = await supabase.from('inventory_order_items').select('id,product_id').eq('order_id', orderId)
...
async function openOrder(order) {
  const requests = [
    supabase.from('inventory_order_items').select('*, inventory_suppliers(name)').eq('order_id', order.id),
    supabase.from('inventory_returns').select('*, inventory_return_items(*)').eq('order_id', order.id).order('created_at', { ascending: false }),
  ]
  if (isSuperAdmin.value) requests.push(supabase.from('inventory_order_item_batch_allocations').select('*, inventory_purchase_batches(batch_no, inventory_suppliers(name))').in('order_item_id', (order.inventory_order_items || []).map((item) => item.id)))
  const [itemRes, returnRes, allocationsRes] = await Promise.all(requests)
  if (itemRes.error || returnRes.error || allocationsRes?.error) return message('订单详情读取失败：' + (itemRes.error?.message || returnRes.error?.message || allocationsRes?.error?.message))
  orderDetail.value = { open: true, order, items: itemRes.data || [], returns: returnRes.data || [], allocations: allocationsRes?.data || [] }
}
...
<template>
  <div class="app-shell inventory-shell inventory-app">
    ...
    <div v-if="orderDetail.open || returnDraft.open || walkInReturn.open || pendingAction.open || flowDetail.open" class="inventory-dialog-backdrop" @click.self="closeInventoryDialog">
      ...
      <section v-else-if="orderDetail.open && !returnDraft.open" class="inventory-dialog inventory-glass-card"><div class="inventory-card-title"><div><p class="eyebrow">ORDER</p><h3>{{ orderDetail.order.order_no }}</h3></div><button class="mini-btn" @click="orderDetail.open=false">关闭</button></div><p class="order-status">{{ orderDetail.order.order_status }} / {{ orderDetail.order.payment_status }} / {{ orderDetail.order.fulfillment_status }}</p><div class="dialog-lines"><p><span>客户 / 联系方式</span><b>{{ orderDetail.order.customer_name || '临时客户' }} {{ orderDetail.order.contact_name || '' }} {{ orderDetail.order.contact_phone || '' }}</b></p><p><span>收货地址</span><b>{{ orderDetail.order.shipping_address || '--' }}</b></p><p><span>配送</span><b>{{ orderDetail.order.shipping_method }} {{ orderDetail.order.carrier || '' }} {{ orderDetail.order.tracking_no || '' }}</b></p><p><span>金额</span><b>商品 {{ money(orderDetail.order.subtotal) }} · 优惠 {{ money(orderDetail.order.discount) }} · 运费 {{ money(orderDetail.order.freight) }} · 应收 {{ money(orderDetail.order.total_amount) }} · 已收 {{ money(orderDetail.order.received_amount) }}</b></p><template v-for="item in orderDetail.items" :key="item.id"><p><span>{{ item.product_name }} {{ item.sku ? `(${item.sku})` : '' }} {{ item.specification || '' }} × {{ item.quantity }}{{ item.inventory_suppliers?.name ? ` · 指定 ${item.inventory_suppliers.name}` : '' }}</span><b>成交 {{ money(item.unit_price) }} · 小计 {{ money(item.line_total) }}</b></p><p v-if="isSuperAdmin" class="order-cost-line"><span>实际成本 {{ money(item.actual_cost ?? (item.unit_cost * item.quantity)) }} · 单位成本 {{ money(item.unit_cost) }} · 毛利 {{ money(item.line_profit) }}</span><b>{{ (orderDetail.allocations || []).filter((allocation) => allocation.order_item_id === item.id).map((allocation) => `${allocation.inventory_purchase_batches?.inventory_suppliers?.name || '批次'} ${allocation.inventory_purchase_batches?.batch_no || ''} × ${allocation.quantity} @ ${money(allocation.unit_cost)}`).join('；') || '待出库后计算' }}</b></p></template><p v-if="isSuperAdmin" class="order-cost-line"><span>订单总成本 / 毛利</span><b>{{ money(orderDetail.order.total_cost) }} / {{ money(orderDetail.order.total_profit) }}</b></p></div><div class="dialog-lines" v-if="orderDetail.returns.length"><p v-for="item in orderDetail.returns" :key="item.id"><span>{{ item.return_no }} · {{ item.reason }}</span><b>退款 {{ money(item.refund_amount) }}</b></p></div><div class="inventory-form-actions"><button v-if="orderDetail.order.fulfillment_status === 'not_fulfilled' && orderDetail.order.order_status !== 'cancelled'" class="glass-primary-btn" :disabled="submitting" @click="fulfillOrder(orderDetail.order)">{{ submitting ? '出库中…' : '确认出库' }}</button><button v-if="orderDetail.order.fulfillment_status === 'fulfilled'" class="glass-primary-btn" :disabled="orderDetail.order.return_status === 'full'" @click="beginOrderReturn">发起退货</button><button class="glass-action-btn" @click="copyBusinessNo(orderDetail.order.order_no)">复制单号</button></div></section>
