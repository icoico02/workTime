...
const sections = [['dashboard', '总览'], ['checkout', '开单'], ['products', '商品'], ['inbound', '入库'], ['outbound', '出库'], ['returns', '退货'], ['stock', '库存'], ['logs', '流水']]
const currentSection = computed(() => route.params.section || 'dashboard')
const sectionTitle = computed(() => ({ dashboard: '进销存总览', checkout: '快速开单', products: '商品管理', inbound: '入库登记', outbound: '出库登记', returns: '销售退货', stock: '库存状态', logs: '经营流水' }[currentSection.value] || '进销存总览'))
...
const cartTotal = computed(() => Math.max(0, cartSubtotal.value - Number(discount.value || 0)))
...
function blankProduct() { return { id: '', name: '', image_url: '', category: '', sku: '', cost_price: 0, sale_price: 0, stock: 0, low_stock_threshold: 3 } }
function requestKey() {
...
async function saveProduct() {
  ...
  let result
  if (editor.value.id) {
    result = await supabase.rpc('inventory_adjust_product', { p_product_id: editor.value.id, p_stock: value.stock, p_cost_price: value.cost_price, p_sale_price: value.sale_price, p_note: '商品资料修改' })
    if (!result.error) result = await supabase.from('inventory_products').update({ name: value.name, image_url: value.image_url, category: value.category, sku: value.sku, low_stock_threshold: value.low_stock_threshold }).eq('id', editor.value.id)
  } else {
    const { data } = await supabase.auth.getUser()
    result = await supabase.from('inventory_products').insert({ ...value, user_id: data.user?.id })
  }
...
async function completeSale() {
  ...
  submitting.value = true
  try {
    const { error } = await supabase.rpc('inventory_create_sale', { p_items: cart.value.map((p) => ({ product_id: p.id, quantity: p.quantity, unit_price: Number(p.unit_price) })), p_payment_method: payment.value, p_discount: Number(discount.value || 0), p_note: saleNote.value || null, p_request_key: requestKey() })
    if (error) return message(error.message.includes('Insufficient') ? '库存不足，请刷新后检查订单。' : '开单失败：' + error.message)
    cart.value = []; payment.value = 'cash'; discount.value = 0; saleNote.value = ''
    await loadData(); message('销售完成，订单、库存和流水已同步保存。')
  } catch (error) {
...
<template>
  <div class="app-shell inventory-shell inventory-app">
    ...
    <main class="inventory-content">
      <section v-if="errorMessage" class="inventory-glass-card inventory-error"><strong>无法载入进销存</strong><p>{{ errorMessage }}</p><button class="glass-primary-btn" @click="loadData">重新读取</button></section>
      ...
      <template v-else-if="currentSection === 'checkout'">
        ...
        <section class="checkout-layout"><div class="inventory-glass-card"><div class="product-toolbar"><input v-model="search" placeholder="搜索商品或货号" /><select v-model="category"><option value="">全部分类</option><option v-for="item in categories" :key="item">{{ item }}</option></select></div><div class="checkout-product-grid"><button v-for="product in visibleProducts" :key="product.id" class="checkout-product" :disabled="!product.stock" @click="addCart(product)"><img v-if="product.image_url" :src="product.image_url" :alt="product.name" /><span v-else class="product-image-placeholder">{{ product.name.slice(0,1) }}</span><b>{{ product.name }}</b><small>{{ product.stock }} 件可售</small><strong>{{ money(product.sale_price) }}</strong></button><p v-if="!visibleProducts.length" class="empty-state">没有匹配的商品</p></div></div>
          <aside class="inventory-glass-card order-panel"><div class="inventory-card-title"><h3>当前订单</h3><span>{{ cart.length }} 种商品</span></div><p v-if="!cart.length" class="empty-state">点击商品加入订单</p><div class="cart-list"><article v-for="item in cart" :key="item.id"><div><strong>{{ item.name }}</strong><input v-model.number="item.unit_price" type="number" min="0" step="0.01" /></div><div class="quantity-stepper"><button @click="changeQuantity(item,-1)">−</button><b>{{ item.quantity }}</b><button @click="changeQuantity(item,1)">＋</button><span>{{ money(item.quantity * item.unit_price) }}</span></div></article></div><label>收款方式<select v-model="payment"><option value="cash">现金</option><option value="wechat">微信</option><option value="alipay">支付宝</option><option value="other">其他</option></select></label><label>整单优惠<input v-model.number="discount" type="number" min="0" step="0.01" /></label><label>备注<input v-model="saleNote" placeholder="选填" /></label><div class="order-total"><span>合计</span><strong>{{ money(cartTotal) }}</strong><small>预计毛利 {{ money(cartProfit) }}</small></div><button class="glass-primary-btn checkout-submit" :disabled="submitting || !cart.length" @click="completeSale">{{ submitting ? '正在保存…' : '完成销售' }}</button></aside>
        </section>
      </template>
      ...
      <template v-else-if="currentSection === 'inbound'"><div class="inventory-section-heading"><div><p class="eyebrow">INBOUND</p><h2>入库登记</h2></div></div><section class="inventory-glass-card movement-card"><form class="inventory-form" @submit.prevent="submitInbound"><label><span>选择商品</span><select v-model="inbound.product" @change="selectInbound"><option value="">请选择商品</option><option v-for="product in products" :key="product.id" :value="product.id">{{ product.name }} · 当前 {{ product.stock }} 件</option></select></label><label><span>入库数量</span><input v-model.number="inbound.quantity" type="number" min="1" /></label><label><span>实际进货单价</span><input v-model.number="inbound.price" type="number" min="0" step="0.01" /></label><label><span>进货总金额</span><output>{{ money(inboundTotal) }}</output></label><label><span>供应商</span><input v-model="inbound.supplier" placeholder="选填" /></label><label><span>备注</span><input v-model="inbound.note" placeholder="选填" /></label><button class="glass-primary-btn" :disabled="submitting">{{ submitting ? '提交中…' : '确认入库' }}</button></form></section></template>
      <template v-else-if="currentSection === 'outbound'"><div class="inventory-section-heading"><div><p class="eyebrow">OUTBOUND</p><h2>普通出库与损耗</h2></div><button class="glass-action-btn" @click="openSection('checkout')">销售开单</button></div><section class="inventory-glass-card movement-card"><form class="inventory-form" @submit.prevent="submitOutbound"><label><span>出库类型</span><select v-model="outbound.type"><option value="normal_outbound">普通出库</option><option value="loss">损耗</option></select></label><label><span>选择商品</span><select v-model="outbound.product"><option value="">请选择商品</option><option v-for="product in products" :key="product.id" :value="product.id">{{ product.name }} · 当前 {{ product.stock }} 件</option></select></label><label><span>数量</span><input v-model.number="outbound.quantity" type="number" min="1" /></label><label><span>备注</span><input v-model="outbound.note" placeholder="例如：赠品、过期损耗" /></label><button class="glass-primary-btn" :disabled="submitting">{{ submitting ? '提交中…' : '确认出库' }}</button></form></section></template>
    ...
    <div v-if="orderDetail.open || returnDraft.open || walkInReturn.open || pendingAction.open || flowDetail.open" class="inventory-dialog-backdrop">
      ...
      <section v-else-if="orderDetail.open && !returnDraft.open" class="inventory-dialog inventory-glass-card"><div class="inventory-card-title"><div><p class="eyebrow">ORDER</p><h3>{{ orderDetail.order.order_no }}</h3></div><button class="mini-btn" @click="orderDetail.open=false">关闭</button></div><p class="order-status">{{ orderDetail.order.order_status }} / {{ orderDetail.order.payment_status }} / {{ orderDetail.order.return_status }}</p><div class="dialog-lines"><p v-for="item in orderDetail.items" :key="item.id"><span>{{ item.product_name }} × {{ item.quantity }}</span><b>{{ money(item.line_total) }}</b></p></div><div class="dialog-lines" v-if="orderDetail.returns.length"><p v-for="item in orderDetail.returns" :key="item.id"><span>{{ item.return_no }} · {{ item.reason }}</span><b>退款 {{ money(item.refund_amount) }}</b></p></div><button class="glass-primary-btn" :disabled="orderDetail.order.return_status === 'full'" @click="beginOrderReturn">发起退货</button></section>
